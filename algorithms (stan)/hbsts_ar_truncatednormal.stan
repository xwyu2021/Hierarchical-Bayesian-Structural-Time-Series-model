...
//
functions {
  real normal_trunc_rng(real mu, real sigma, real lb, real ub) {
...
model {
  ...
  for(i in 1:(T+T_forecast)){
    for(m in 1:M){
      if(presence[i,m] == 1){
        if(m == 1){
        if(i==1){
          x_obs[1:index_x_t[1,1]] ~ normal(x[1,1],sigma_obsx[m]) T[0,36];
        }else{
          x_obs[(index_x_t[i-1,M]+1):index_x_t[i,1]] ~ normal(x[i,1],sigma_obsx[m]) T[0,36];
        }
      }else{
        x_obs[(index_x_t[i,m-1]+1):index_x_t[i,m]] ~ normal(x[i,m],sigma_obsx[m]) T[0,36];
      }
    }
  }
  }
  ...
  alpha ~ cauchy(0,1);
  for(i in 1:T){
    y[index_obs_t[i]:(index_obs_t[i+1]-1)] ~ normal(alpha[i],sigma_obs) T[0,36];
  }
  for(i in 1:T){
...
generated quantities {
  ...
  for(i in 1:T_forecast){
    for(j in index_obs_forecast[i]:(index_obs_forecast[i+1]-1)){
      ys[j] = normal_trunc_rng(alpha_forecast_unscale[i], sigma_obs, 0, 36);
      //ys[j] = normal_rng(alpha_forecast_unscale[i],sigma_obs) T[0,36];
